...
require_once($CFG->dirroot.'/mod/questionnaire/classes/question/insrate.php');
...
abstract class question {
    ...
    private function get_choices() {
        ...
        if ($qtype == 'Instructor List') {
            ...
            $teachers = get_role_users($teacherroleid, $context);
            if ($teachers) {
                ...
                foreach ($teachers as $teacher) {
                    ...
                    if (!$insrecord) {
                        ...
                        $choiceins->userid = $USER->id;
                        $choiceins->question_id = $this->id;
                        $choiceins->staffid = $teacherid;
                        $serviceid = $DB->insert_record('questionnaire_quest_ins', $choiceins);
                    }
                }
                if ($choices = $DB->get_records('questionnaire_quest_ins',
                    ...
                    foreach ($choices as $choice) {
                        ...
                        $this->choices[$choice->id] = array('id' => $choice->id, 'questionid' => $this->id,
                                        'content' => $lname, 'value' => '');
                    }
                }
            }
    ...
    protected function form_choices(\MoodleQuickForm $mform) {
        if ($this->has_choices()) {
            ...
            if ( $this->type_id <> 11) {
                 ...
                 $mform->addElement('hidden', 'allchoices', get_string('possibleanswers', 'questionnaire'), $options);
                 $mform->setType('allchoices', PARAM_RAW);
            }
